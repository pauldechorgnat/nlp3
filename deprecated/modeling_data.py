...
def building_data_silent(path):
	...
	vocabulary = Counter(vocabulary_full)
	vocabulary = sorted(vocabulary, key = vocabulary.get, reverse = True)
	mapper = {token:index+1 for index, token in enumerate(vocabulary)}
	mapper['<unk>'] = len(mapper)+1
	mapper['<pad>'] = len(mapper)+1
...
def building_data_test_silent(path):
    """function to build the dictionnary of the data"""
    print('loading text ...')
    list_of_attributes = []
    # opening the file
...
def parsing_attributes_test(attribute):
    # defining dictionnaries to change data into integers and then arrays
    food_dict = {'Chinese': 4, 'English': 1, 'Fast food': 5, 'French': 2, 'Indian': 3, 'Italian': 6, 'Japanese': 0}
    eatType_dict = {'coffee shop': 0, 'pub': 1, 'restaurant': 2}
    ratings_dict = {'1 out of 5': 0, '3 out of 5': 1, '5 out of 5': 2, 'average': 1, 'high': 2, 'low': 0}
    ...
    # for key in [ 'eatType', 'food', 'priceRange', 'customerrating', 'familyFriendly']:
    # 	dictionnary.pop(key)

    return dummy_array, dictionnary
...
def parsing_list_of_attributes_silent(list_of_attributes):
	return_array = [parsing_attributes(att) for att in list_of_attributes]
	return_array = np.array(return_array)#.reshape(len(list_of_attributes), -1)
	return return_array
...
def parsing_list_of_attributes_test_silent(list_of_attributes):
	attributes_list = []
	dictionnary_list = []
...
def padding_review_silent(list_of_reviews, padding_length):
	"""review is a list of tokens"""
	list_of_padded_reviews = []
...
def mapping_reviews_silent(list_of_reviews, mapper, review_len = 76):
	nb_of_samples = len(list_of_reviews)
	mapped_tokens = []
